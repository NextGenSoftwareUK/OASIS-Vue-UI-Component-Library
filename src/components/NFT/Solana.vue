...
import { ref } from 'vue'
const emit = defineEmits<{ close: []; submit: [{ solana: string; provider: string; toAvatar: string; walletAddress: string; message: string }] }>()
const form = ref({ solana: '', provider: 'Solana', toAvatar: '', walletAddress: '', message: '' })
function handleSubmit() { emit('submit', { ...form.value }) }
</script>
<template>
  <div class="p-shell">
    <h2 class="p-title">◎ Solana Provider</h2>
    <form class="p-form" @submit.prevent="handleSubmit">
      <div class="p-row"><label class="p-label">Solana</label><input class="p-input" v-model="form.solana" placeholder="Solana address or amount" /></div>
      <div class="p-row">
        <label class="p-label">Provider</label>
        <select class="p-input" v-model="form.provider">
          <option>Solana</option><option>Ethereum</option><option>Holochain</option>
        </select>
      </div>
      <div class="p-row"><label class="p-label">To Avatar</label><input class="p-input" v-model="form.toAvatar" placeholder="Recipient avatar username" /></div>
      <div class="p-row"><label class="p-label">Or Wallet Address</label><input class="p-input" v-model="form.walletAddress" placeholder="0x... or Solana address" /></div>
      <div class="p-row"><label class="p-label">Message</label><textarea class="p-textarea" v-model="form.message" placeholder="Optional message..."></textarea></div>
      <div class="p-actions"><button class="p-btn-cancel" type="button" @click="emit('close')">Cancel</button><button class="p-btn" type="submit">Send</button></div>
    </form>
  </div>
</template>
<style scoped>
