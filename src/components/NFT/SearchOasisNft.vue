...
import { ref } from 'vue'
const emit = defineEmits<{ close: []; submit: [{ query: string; provider: string; type: string }] }>()
const form = ref({ query: '', provider: 'All', type: 'All' })
function handleSubmit() { emit('submit', { ...form.value }) }
</script>
<template>
  <div class="p-shell">
    <h2 class="p-title">🔍 Search OASIS NFTs</h2>
    <form class="p-form" @submit.prevent="handleSubmit">
      <div class="p-row"><label class="p-label">Name / ID</label><input class="p-input" v-model="form.query" placeholder="Search NFTs..." /></div>
      <div class="p-row">
        <label class="p-label">Provider</label>
        <select class="p-input" v-model="form.provider">
          <option>All</option><option>Ethereum</option><option>Solana</option><option>Polygon</option>
        </select>
      </div>
      <div class="p-row">
        <label class="p-label">Type</label>
        <select class="p-input" v-model="form.type">
          <option>All</option><option>Avatar</option><option>Land</option><option>Item</option><option>Art</option>
        </select>
      </div>
