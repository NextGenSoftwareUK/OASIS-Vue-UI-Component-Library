...
import { ref } from 'vue'
defineProps<{ initialName?: string; initialDescription?: string }>()
const emit = defineEmits<{ close: []; submit: [{ name: string; description: string; version: string }] }>()
const form = ref({ name: '', description: '', version: '1.0.0' })
function handleSubmit() { emit('submit', { ...form.value }) }
</script>
<template>
  <div class="p-shell">
    <h2 class="p-title">✏️ Edit OAPP</h2>
    <form class="p-form" @submit.prevent="handleSubmit">
      <div class="p-row"><label class="p-label">OAPP Name</label><input class="p-input" v-model="form.name" :placeholder="initialName ?? 'OAPP name'" /></div>
      <div class="p-row"><label class="p-label">Description</label><textarea class="p-textarea" v-model="form.description" :placeholder="initialDescription ?? 'Description'"></textarea></div>
      <div class="p-row"><label class="p-label">Version</label><input class="p-input" v-model="form.version" placeholder="1.0.0" /></div>
