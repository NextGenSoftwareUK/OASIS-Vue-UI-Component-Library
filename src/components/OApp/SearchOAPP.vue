...
import { ref } from 'vue'
const emit = defineEmits<{ close: []; submit: [{ query: string; type: string; provider: string }] }>()
const form = ref({ query: '', type: 'All', provider: 'All' })
function handleSubmit() { emit('submit', { ...form.value }) }
</script>
<template>
  <div class="p-shell">
    <h2 class="p-title">🔍 Search OAPPs</h2>
    <form class="p-form" @submit.prevent="handleSubmit">
      <div class="p-row"><label class="p-label">OAPP Name</label><input class="p-input" v-model="form.query" placeholder="Search OAPPs..." /></div>
      <div class="p-row">
        <label class="p-label">Type</label>
        <select class="p-input" v-model="form.type">
          <option>All</option><option>Web</option><option>Game</option><option>AR</option><option>VR</option><option>Mobile</option>
        </select>
      </div>
      <div class="p-row">
